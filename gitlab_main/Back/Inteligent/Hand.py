...
class SCV():
    ...
    def restratOld(self,new): 
        if abs(self.oldDistance-new)>=10 : self.oldDistance = new 
    
    def detection(self):
        if self.oldDistance >=350 : self.classe.fonction_SCV(-1)
        elif self.oldDistance>=200 : self.classe.fonction_SCV(0.02)
        elif self.oldDistance >=60: self.classe.fonction_SCV(0.01)
