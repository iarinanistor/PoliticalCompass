...
def election_multiple(map_carrer, start, end, k, type_votes):
    ...
    for i in range(start, end, k):
        ...
        l.append(map_carrer.Monte_carlo_vote(type_votes))
    return l
...
def CreationSimulation(map, k,type_votes):
    """
    ...
    """
    copie_map = copy.deepcopy(map) # Bug genere avec le GUI sinon source du probleme dependenace entre les bibluotheques 
    mc = Monte_Carlo(copie_map, k,type_votes)
    x, y = moyenne_couples(mc)
    
    # Ajout d'un candidat test basé sur la moyenne des positions
    copie_map.liste_electeur.append(Candidat("test", "test", 100, 100, x, y))
    
    # Exécution et affichage des résultats de la méthode Copeland
    for i in range(1, 5):
        gg =  copie_map.Copeland()
        print(f"test {i}", gg.nom())
    
    # Création et placement sur la HitMap
    hitmap = HitMap( copie_map.generationX)
    hitmap.placeALL( copie_map, mc, (x, y))
    return hitmap
...
def SimulationAleatoire(ModeVotes, t, k,type_votes):
    ...
    map.liste_electeur.append(Candidat("test", "test", 100, 100, 0, 0))
    map.generationTest(15, 25)
    return CreationSimulation(map,k,type_votes)
