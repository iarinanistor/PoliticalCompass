...
class SCV():
    ...
    def restratOld(self,new): 
        if abs(self.oldDistance-new)>=20 : self.oldDistance = new 
    
    def detection(self):
        if self.oldDistance >=300 : self.classe.fonction_SCV(1)
        elif self.oldDistance>=200 : self.classe.fonction_SCV(0.02)
        elif self.oldDistance >=100: self.classe.fonction_SCV(0.01)
        ...
        while cap.isOpened():
            ...
            if results.multi_hand_landmarks:
                for hand_landmarks in results.multi_hand_landmarks:
                    ...
                    slh = SLH(Main)
                    distance_point_pouce =slh.lisse(self.oldDistance)
                    self.detection()
