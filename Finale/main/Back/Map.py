...
from Calcul_Emissions import emission_moyen_copeland, emission_moyen_borda, emission_moyen_pluralite, emission_moyen_stv, emission_moyen_approbation
...
class Map:
    ...
    def Copeland(self, r=None):
        ...
        bd.conso += emission_moyen_copeland
    ...
    def Pluralite(self,r):
        ...
        #Ajout de la consommation
        bd.conso += emission_moyen_pluralite
    ...
    def Approbation(self,nb_approbation, r=None):
        ...
        #Ajout de la consommation
        bd.conso += emission_moyen_approbation
    ...
    def delegation(self, electeurs_proximite):
        '''
        Paramaters:
        ...
        if nomFichier == "":
            ...
            with open(nomFichier, "w") as fichier:
                ...
                for liste_ind in self.population:
                    for ind in liste_ind:
                        if ind != None: 
                            fichier.write(str(ind.nom) + " " + str(ind.x) + " " + str(ind.y) + "\n")
    ...
    def zone(self, ind, r):
        ...
        for x in range(max(0, ind.x - r), min(self.generationX, ind.x + r + 1)):
            for y in range(max(0, ind.y - r), min(self.generationY, ind.y + r + 1)):
                print(ind.nom, self.population[x][y].nom)
                if (((x != ind.x or y != ind.y) and (self.population[x][y].poids >= 1)) and (ind.nom != self.population[x][y].nom)):
